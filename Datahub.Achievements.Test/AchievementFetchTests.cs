...
public class AchievementFetchTests
{
    ...
    private static readonly Dictionary<string, UserAchievement> FakeAchievements = new()
    {
        {
    ...
    public async Task GetUsersAchievements()
    {
        ...
        mockStorage.Setup(s => s.GetItemAsync<Dictionary<string, UserAchievement>>(It.IsAny<string>(), null))
            .ReturnsAsync(FakeAchievements);

        var achievementService = new AchievementService(mockLogger.Object, mockStorage.Object, Options);

        var result = await achievementService!.GetUserAchievements(UserId);

        Assert.That(result, Is.TypeOf<Dictionary<string, UserAchievement>>());
        Assert.That(result, Has.Count.EqualTo(FakeAchievements.Count));
    }
    ...
    public async Task GetEarnedUsersAchievements()
    {
        ...
        Assert.That(result, Is.TypeOf<Dictionary<string, UserAchievement>>());
        Assert.That(result.Count(r => r.Value.Earned), Is.EqualTo(FakeAchievements.Count(r => r.Value.Earned)));
    }
}