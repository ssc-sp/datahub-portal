...
public class ProjectUserManagementService : IProjectUserManagementService
{
    ...
    {
        _logger.LogInformation("Preparing to add user {UserGraphId} to project {ProjectAcronym}", userGraphId, projectAcronym);
        ...
        if (project == null)
        {
            _logger.LogError("Project {ProjectAcronym} not found when trying to add user {UserGraphId}", projectAcronym, userGraphId);
            throw new Exception($"Project {projectAcronym} not found");
        }
        
        var exists = project.Users.Any(u => u.User_ID == userGraphId);
        if(exists)
        {
            _logger.LogInformation("User {UserGraphId} already exists in project {ProjectAcronym}", userGraphId, projectAcronym);
            return;
        }

        var approvingUser = await _userInformationService.GetUserIdString();
        _logger.LogInformation("Adding user {UserGraphId} to project {ProjectAcronym} by user {ApprovingUser}", userGraphId, projectAcronym, approvingUser);
