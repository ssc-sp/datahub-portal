...
{
    public class HealthCheckConsumer : IConsumer<InfrastructureHealthCheckMessage>
    {
        ...
        private readonly AzureDevOpsConfiguration _config = new AzureDevOpsConfiguration();
        ...
        public async Task Consume(ConsumeContext<InfrastructureHealthCheckMessage> context)
        {
            ...
            if (healthCheckMessage.Group == "all")
            {
                await RunAllChecks();
            }
            else
            {
                var request = new InfrastructureHealthCheckRequest(healthCheckMessage.Type, healthCheckMessage.Group, healthCheckMessage.Name);
        ...
        private async Task RunHealthCheck(InfrastructureHealthCheckRequest request)
        {
        ...
        private async Task StoreHealthCheckRun(InfrastructureHealthCheckResponse result)
        {
            ...
            _projectDBContext.InfrastructureHealthCheckRuns.Add(check);
            await _projectDBContext.SaveChangesAsync();
        }
