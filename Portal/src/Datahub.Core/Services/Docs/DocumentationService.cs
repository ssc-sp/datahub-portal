...
public class DocumentationService
{
    ...
    private async Task LoadResourceTree(DocumentationGuideRootSection guide, bool useCache = true)
    {
        ...
        enOutline = SidebarParser.ParseSidebar(guide, await LoadDocsPage(guide, SIDEBAR, LOCALEEN, useCache), docFileMappings.GetEnglishDocumentId);
        if (enOutline is null)
            throw new InvalidOperationException("Cannot load sidebar and content");

        frOutline = SidebarParser.ParseSidebar(guide, await LoadDocsPage(guide, $"{SIDEBAR}", LOCALEFR, useCache), docFileMappings.GetFrenchDocumentId);
