...
public class ServiceAuthManager
{
    ...
    internal List<string> GetAllProjects()
    {
        ...
        return ctx.Projects.Where(p => p.ProjectAcronymCD != null).Select(p => p.ProjectAcronymCD).ToList();
    }
    ...
    public static readonly Regex EmailExtractor = new Regex(".*<(.*@.*)>", RegexOptions.Compiled);
    ...
    public static string ExtractEmail(string input)
    {
        if (EmailRegex.IsMatch(input))
            return input;
        var match = EmailExtractor.Match(input);
        if (match.Success)
        {
            var ingroup = match.Groups[1].Value.ToLowerInvariant();
            if (EmailRegex.IsMatch(ingroup))
                return ingroup;
        }
    ...
    public List<string> GetProjectAdminsEmails(string projectAcronym)
    {
        ...
        return ctx.ProjectUsers
            .Where(a =>
                a.Project.ProjectAcronymCD == projectAcronym
                && (a.RoleId == (int)ProjectRole.RoleNames.Admin ||
                    a.RoleId == (int)ProjectRole.RoleNames.WorkspaceLead)
    ...
    {
        using var ctx = dbFactory.CreateDbContext();
        var mailboxEmails = ctx.Projects.Where(u => u.ProjectAcronymCD == projectAcronym).Select(s => s.ProjectAdmin)
            .FirstOrDefault();
    ...
    private async Task<Dictionary<string, List<string>>> CheckCacheForAdmins()
    {
        ...
        if (!serviceAuthCache.TryGetValue(PROJECT_ADMIN_KEY, out allProjectAdmins))
        {
            ...
            var adminsFromProjectUsersTable = await ctx.ProjectUsers
                .AsNoTracking()
                .Include(a => a.Project)
                .Include(a => a.PortalUser)
                .Where(u =>
                    u.RoleId == (int)ProjectRole.RoleNames.Admin
                    || u.RoleId == (int)ProjectRole.RoleNames.WorkspaceLead)
                .ToListAsync();

            foreach (var admin in adminsFromProjectUsersTable)
            {
                if (allProjectAdmins.TryGetValue(admin.Project.ProjectAcronymCD, out var projectAdmin))
                {
                ...
                else
                {
                    allProjectAdmins.Add(
                        admin.Project.ProjectAcronymCD,
    ...
    public async Task<ImmutableList<(ProjectRole Role, DatahubProject Workspace)>> GetUserAuthorizations(string userGraphId)
    {
        if (serviceAuthCache.TryGetValue(
            AUTH_KEY,
            out Dictionary<string, List<(ProjectRole, DatahubProject)>> usersAuthorization))
        {
        ...
        var usersRoles = await ctx.ProjectUsers
            .AsNoTracking()
            .Include(a => a.Project)
            .Include(a => a.PortalUser)
            .Include(a => a.Role)
        ...
        {
            return ImmutableList<(ProjectRole, DatahubProject)>.Empty;
        }
