...
public class UserInformationService : IUserInformationService
{
    ...
    public async Task<bool> IsUserProjectAdmin(string projectAcronym)
    {
        ...
        if (await IsUserInDataHubAdminRole())
            return true;
        return (await GetAuthenticatedUser()).IsInRole($"{projectAcronym}{RoleConstants.ADMINSUFFIX}");
    }
    ...
    public async Task<bool> IsUserProjectWorkspaceLead(string projectAcronym)
    {
        ...
        if (await IsUserInDataHubAdminRole())
            return true;
        return (await GetAuthenticatedUser()).IsInRole($"{projectAcronym}{RoleConstants.WORKSPACELEADSUFFIX}");
    }

    public async Task<bool> IsUserDatahubAdmin()
    {
        return (await GetAuthenticatedUser()).IsInRole(RoleConstants.DATAHUBROLEADMIN);
    }
    ...
    public async Task CreatePortalUserAsync(string userGraphId)
    {
        ...
        try
        {
            ...
            {
                ObjectType = Model.Catalog.CatalogObjectType.User,
                ObjectId = userGraphId,
                NameEnglish = graphUser.DisplayName,
                NameFrench = graphUser.DisplayName,
                DescEnglish = graphUser.Department,
                DescFrench = graphUser.Department
            };
